...
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.0/js/bootstrap.min.js"
    ...
    postRender : async () => {
        ...
        function isInterviewDate(date, month, year) {
            let isValidDate = false;
            if (interviewsData.length > 0) {
                interviewsData.forEach(function (obj) {
                    for (var k in obj) {
                        if (k === 'interview_date') {
                            var stringDate = obj[k];
                        }
                    }
                    let dateArr = stringDate.split("-");
                    let completeDate = new Date(dateArr[0], dateArr[1], dateArr[2]);
                    if (completeDate.getDate() === date &&
                        (completeDate.getMonth() - 1) === month &&
                        completeDate.getFullYear() === year){
                        isValidDate = true;
                    }
                });
            }
